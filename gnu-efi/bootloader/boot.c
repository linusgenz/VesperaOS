                   ...
                   EFI_FILE** OutDirectory)
{
    ...
    if (Directory == NULL)
    {
        ...
        if (OutDirectory) *OutDirectory = Directory;
    }
    else
    {
        if (OutDirectory) *OutDirectory = NULL;
    }
...
{
    EFI_FILE* fontDir = NULL;
    EFI_FILE* fontFile = LoadFile(Directory, Path, ImageHandle, SysTbl, &fontDir);
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE* SysTbl)
{
    ...
    EFI_FILE* Kernel = LoadFile(NULL, KERNEL_PATH, ImageHandle, SysTbl, &KernelDir);
    if (Kernel == NULL)
    {
    ...
    status = SysTbl->BootServices->AllocatePool(EfiLoaderData, MapSize, (void**)&Map);
    if (EFI_ERROR(status))
    {
        Print(L"Failed to allocate memory for MemoryMap\n");
    }
    status = SysTbl->BootServices->GetMemoryMap(&MapSize, Map, &MapKey, &DescriptorSize, &DescriptorVersion);
    if (EFI_ERROR(status))
    {
        Print(L"Failed to get MemoryMap\n");
    }
    ...
    // 2) Puffer reservieren (+ etwas Puffer)
    MapSize += 2 * DescriptorSize;
    status = SysTbl->BootServices->AllocatePool(EfiLoaderData, MapSize, (void**)&Map);
    if (EFI_ERROR(status))
    {
    ...
    while (1)
    {
        UINTN tmpMapSize = MapSize;
        status = SysTbl->BootServices->GetMemoryMap(&tmpMapSize, Map, &MapKey, &DescriptorSize, &DescriptorVersion);
        if (EFI_ERROR(status))
        {
        ...
        status = SysTbl->BootServices->ExitBootServices(ImageHandle, MapKey);
        if (!EFI_ERROR(status)) break;

        if (!((INTN)status == (INTN)EFI_INVALID_PARAMETER))
        {
