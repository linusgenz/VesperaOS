...
void heapsort(RandomIt first, RandomIt last, Compare comp) {
    ...
    for (diff_t i = n / 2 - 1; i >= 0; --i) {
        diff_t parent = i;
        auto val = klib::move(first[parent]);
        ...
        while (true) {
            ...
            first[parent] = klib::move(first[child]);
            parent = child;
        }
        first[parent] = klib::move(val);
    }

    // Extract elements
    for (diff_t i = n - 1; i > 0; --i) {
        klib::swap(first[0], first[i]);
