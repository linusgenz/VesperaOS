    ...
    do {                                                                          \
        ...
        if (!(_e == _a)) {                                                        \
            ...
            snprintf(                                                             \
                _buf,                                                             \
                sizeof(_buf),                                                     \
                "ASSERT_EQ: expected=%s, got=%s  [" #expected " == " #actual "]", \
...
#define ASSERT_NE(a, b)                                                                    \
    do {                                                                                   \
        if ((a) == (b)) {                                                                  \
            ::TestFramework::fail_test(__FILE__, __LINE__, "ASSERT_NE(" #a " != " #b ")"); \
            return;                                                                        \
        }                                                                                  \
    } while (0)
...
#define ASSERT_NULL(ptr)                                                             \
    do {                                                                             \
        if ((ptr) != nullptr) {                                                      \
            ::TestFramework::fail_test(__FILE__, __LINE__, "ASSERT_NULL(" #ptr ")"); \
            return;                                                                  \
        }                                                                            \
    } while (0)
...
#define ASSERT_NOT_NULL(ptr)                                                             \
    do {                                                                                 \
        if ((ptr) == nullptr) {                                                          \
            ::TestFramework::fail_test(__FILE__, __LINE__, "ASSERT_NOT_NULL(" #ptr ")"); \
            return;                                                                      \
        }                                                                                \
    } while (0)
...
#define ASSERT_MEM_EQ(expected_ptr, actual_ptr, size)                                                       \
    do {                                                                                                    \
        if (memcmp((expected_ptr), (actual_ptr), (size)) != 0) {                                            \
            ::TestFramework::fail_test(__FILE__, __LINE__, "ASSERT_MEM_EQ: Speicher stimmt nicht überein"); \
            return;                                                                                         \
        }                                                                                                   \
    } while (0)
...
#define ASSERT_STR_EQ(expected, actual)                                                                       \
    do {                                                                                                      \
        if (strcmp((expected), (actual)) != 0) {                                                              \
            char _buf[256];                                                                                   \
            snprintf(_buf, sizeof(_buf), "ASSERT_STR_EQ: expected=\"%s\", got=\"%s\"", (expected), (actual)); \
            ::TestFramework::fail_test(__FILE__, __LINE__, _buf);                                             \
            return;                                                                                           \
        }                                                                                                     \
    } while (0)
...
#define ASSERT_GE(a, b)                                                                 \
    do {                                                                                \
        if (!((a) >= (b))) {                                                            \
            char _buf[128];                                                             \
            snprintf(_buf, sizeof(_buf), "ASSERT_GE: %s >= %s fehlgeschlagen", #a, #b); \
            ::TestFramework::fail_test(__FILE__, __LINE__, _buf);                       \
            return;                                                                     \
        }                                                                               \
    } while (0)
...
#define ASSERT_LE(a, b)                                                                 \
    do {                                                                                \
        if (!((a) <= (b))) {                                                            \
            char _buf[128];                                                             \
            snprintf(_buf, sizeof(_buf), "ASSERT_LE: %s <= %s fehlgeschlagen", #a, #b); \
            ::TestFramework::fail_test(__FILE__, __LINE__, _buf);                       \
            return;                                                                     \
        }                                                                               \
    } while (0)
...
namespace TestFramework {
    ...
    inline void register_test(const char* suite, const char* name, std::function<void()> fn) {
    ...
    inline int run_all_tests(const char* filter_suite = nullptr) {
        ...
        for (auto& t : registry()) {
            ...
            if (!current_test_failed) {
                ...
                printf("  " TF_RED "FAIL" TF_RESET "  %s\n%s\n", t.name.c_str(), current_failure_msg);
                failed++;
            }
        }
        ...
        printf("\n" TF_BOLD "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
        printf(
            "  Ergebnis: " TF_GREEN "%d bestanden" TF_RESET "  " TF_RED "%d fehlgeschlagen" TF_RESET
            "  %d übersprungen\n" TF_RESET,
            passed,
            failed,
            skipped
...
#define TEST(suite_name, test_name)                                                             \
    static void _test_##suite_name##_##test_name();                                             \
    static bool _reg_##suite_name##_##test_name = [] {                                          \
        TestFramework::register_test(#suite_name, test_name, _test_##suite_name##_##test_name); \
        return true;                                                                            \
    }();                                                                                        \
    static void _test_##suite_name##_##test_name()
