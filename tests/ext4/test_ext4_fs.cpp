...
TEST(Ext4_FS, ValidImageIsValid,
     "FileSystem mounted from test_ext4.img reports is_valid() == true") {
    WITH_EXT4(f);
    ASSERT_TRUE(f.fs->is_valid());
}

TEST(Ext4_FS, InvalidAllZeroImageIsNotValid,
     "FileSystem on a zeroed block device is not valid") {
...
TEST(Ext4_FS, InvalidAllFFImageIsNotValid,
     "FileSystem on a 0xFF-filled block device is not valid") {
    auto* mdev = new MockBlockDevice(256);
...
TEST(Ext4_FS, InvalidSmallDeviceIsNotValid,
     "A device too small to hold even a superblock is not valid") {
...
TEST(Ext4_FS, SuperblockMagicIsCorrect,
     "get_superblock()->s_magic == EXT4_MAGIC (0xEF53)") {
    WITH_EXT4(f);
    ASSERT_EQ(static_cast<u16>(ext4::EXT4_MAGIC),
              f.fs->get_superblock()->s_magic);
}

TEST(Ext4_FS, SuperblockLabelIsVespTest,
     "Filesystem volume label matches the one set by mkfs.ext4") {
    WITH_EXT4(f);
    // The label is "vesp_test" (9 bytes); s_volume_name is 16 bytes.
    ASSERT_EQ(0, strncmp(reinterpret_cast<const char*>(
                              f.fs->get_superblock()->s_volume_name),
                          "vesp_test", 9));
}

TEST(Ext4_FS, SuperblockInodeCountIsPositive,
     "s_inodes_count is greater than zero") {
    WITH_EXT4(f);
    ASSERT_TRUE(f.fs->get_superblock()->s_inodes_count > 0u);
}

TEST(Ext4_FS, SuperblockBlockCountIsPositive,
     "s_blocks_count_lo is greater than zero") {
    WITH_EXT4(f);
    ASSERT_TRUE(f.fs->get_superblock()->s_blocks_count_lo > 0u);
}


TEST(Ext4_FS, BlockSizeIs4096,
     "get_block_size() returns 4096 (mkfs -b 4096)") {
    WITH_EXT4(f);
    ASSERT_EQ(static_cast<u32>(4096), f.fs->get_block_size());
}

TEST(Ext4_FS, BlockSizeIsPowerOfTwo,
     "Block size is a power of two and at least 1024") {
    WITH_EXT4(f);
...
TEST(Ext4_FS, ProbeDetectsExt4,
     "ext4_driver.probe() returns 1 for a valid ext4 image") {
    WITH_EXT4(f);
...
TEST(Ext4_FS, ProbeExtractsLabel,
     "ext4_driver.probe() copies the volume name into FilesystemInfo::label") {
    WITH_EXT4(f);
    FilesystemInfo info = {};
...
TEST(Ext4_FS, ProbeRejectsZeroedDevice,
     "ext4_driver.probe() returns 0 for a zeroed device") {
    auto* mdev = new MockBlockDevice(256);
    FilesystemInfo info = {};
...
TEST(Ext4_FS, RootInodeIsTwo,
     "The internal_data of the root VfsNode has inode == EXT4_ROOT_INODE (2)") {
    WITH_EXT4(f);
    auto* nd = static_cast<Ext4Node*>(f.root->internal_data);
    ASSERT_EQ(static_cast<u32>(ext4::EXT4_ROOT_INODE), nd->inode);
}

TEST(Ext4_FS, RootNodeIsDirectory,
     "The root VfsNode type is Directory") {
    WITH_EXT4(f);
    ASSERT_EQ(static_cast<int>(VfsNodeType::Directory),
              static_cast<int>(f.root->type));
}

TEST(Ext4_FS, RootNodeIsPermanent,
     "The root VfsNode has permanent == true") {
    WITH_EXT4(f);
    ASSERT_TRUE(f.root->permanent);
}

TEST(Ext4_FS, ReadDirectoryRootHasExpectedEntries,
     "Root directory contains at least the known image files") {
    WITH_EXT4(f);

    usize count = 0;
    ext4::FileEntry* entries = f.fs->read_directory(ext4::EXT4_ROOT_INODE, count);
    ASSERT_NOT_NULL(entries);

    // Minimum entries: ., .., hello.txt, empty.txt, binary.bin, big.bin,
    //                  trunctest.txt, subdir, deep
    ASSERT_GE(count, static_cast<usize>(9));
    kernel::memory::free(entries);
}

TEST(Ext4_FS, ReadDirectoryListMatchesFindPath,
     "Every name returned by read_directory() can be found via find()") {
    WITH_EXT4(f);
    ...
    for (usize i = 0; i < count; ++i) {
        ...
        // Skip . and .. — find() on root for these returns a node but it's
        // not the root itself in all implementations; just verify non-special.
...
TEST(Ext4_FS, FindDeepDirectoryLevel1,
     "find() can locate the 'deep' directory in root") {
    WITH_EXT4(f);
    EXT4_FIND(f, "deep", deep);
    NodeGuard guard(deep);

    ASSERT_EQ(static_cast<int>(VfsNodeType::Directory),
              static_cast<int>(deep->type));
}

TEST(Ext4_FS, FindDeepDirectoryLevel2,
     "find() can locate 'deep/level1' by chaining two find() calls") {
    WITH_EXT4(f);
    EXT4_FIND(f, "deep", deep);
    ...
    ASSERT_EQ(static_cast<int>(VfsNodeType::Directory),
              static_cast<int>(level1->type));

    Ext4Fixture::free_node(level1);
    Ext4Fixture::free_node(deep);
}

TEST(Ext4_FS, FindDeepLeafFile,
     "find() can locate 'deep/level1/leaf.txt' via chained finds") {
    WITH_EXT4(f);
...
TEST(Ext4_FS, StressCreateWriteReadDelete10Files,
     "Create, write, read, and delete 10 files without errors") {
    WITH_EXT4(f);

    constexpr int N = 10;
    char names[N][24];
    for (int i = 0; i < N; ++i)
        snprintf(names[i], sizeof(names[i]), "stress_%02d.txt", i);

    // Create
    for (int i = 0; i < N; ++i)
        ASSERT_EQ(0, f.create(f.root, names[i]));
    ...
    // Delete
    for (int i = 0; i < N; ++i)
        ASSERT_EQ(0, f.unlink(f.root, names[i]));

    // Verify absence
    auto listing = f.list_root();
    for (int i = 0; i < N; ++i)
        ASSERT_FALSE(f.list_contains(listing, names[i]));
}

TEST(Ext4_FS, StressCreateManyFilesInSubdir,
     "Creating 30 files inside a subdirectory all appear in the listing") {
    WITH_EXT4(f);
...
TEST(Ext4_FS, StressMixedOperations,
     "Mix of create, write, read, rename, unlink is consistent") {
    WITH_EXT4(f);
...
TEST(Ext4_FS, StressMultiBlockWriteReadCycles,
     "5 cycles of write-8192-bytes + full-read produce consistent results") {
    WITH_EXT4(f);
    ...
    for (int cycle = 0; cycle < 5; ++cycle) {
        ...
        for (usize i = 0; i < 8192; ++i)
            src[i] = static_cast<uint8_t>((i + cycle) & 0xFF);
