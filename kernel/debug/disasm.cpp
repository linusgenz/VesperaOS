...
static const char *reg64[] = {"rax", "rcx", "rdx", "rbx", "rsp", "rbp",
                              "rsi", "rdi", "r8",  "r9",  "r10", "r11",
                              "r12", "r13", "r14", "r15"};
static const char *reg32[] = {"eax",  "ecx",  "edx",  "ebx", "esp",  "ebp",
                              "esi",  "edi",  "r8d",  "r9d", "r10d", "r11d",
                              "r12d", "r13d", "r14d", "r15d"};
static const char *reg16[] = {"ax",   "cx",   "dx",   "bx",  "sp",   "bp",
                              "si",   "di",   "r8w",  "r9w", "r10w", "r11w",
                              "r12w", "r13w", "r14w", "r15w"};
static const char *reg8[] = {"al",   "cl",   "dl",   "bl",  "spl",  "bpl",
...
size_t decode_rm_operand(const uint8_t *code, size_t offset, size_t max_len,
                         ModRM modrm, REXPrefix rex, int operand_size,
                         uint64_t instr_addr, size_t instr_len, char *output,
                         size_t output_size, bool show_size_prefix = true) {
  ...
  if (modrm.mod == 0) {
    ...
    if (modrm.rm == 5) {
      ...
      Symbol s = lookup_symbol(target);
      uint64_t offset = target - s.addr;

      snprintf(output, output_size, "%s[rip%s] -> 0x%llx <%.*s+0x%llx>",
               size_prefix, disp_str, target, (int)s.len, s.name, offset);
      return 4;
    }
    ...
    if (modrm.rm == 4) {
      ...
      if ((sib & 7) == 5) {
        ...
        if (index == 4)
          ...
          snprintf(output, output_size, "%s[%s*%d%s]", size_prefix,
                   get_reg_name(index, 8), scale, disp_str);
        return 5; // 1 SIB + 4 disp
      }

      //  SIB without disp32
      if (index == 4)
        snprintf(output, output_size, "%s[%s]", size_prefix,
                 get_reg_name(base, 8));
      else
        snprintf(output, output_size, "%s[%s + %s*%d]", size_prefix,
                 get_reg_name(base, 8), get_reg_name(index, 8), scale);
  ...
  if (modrm.mod == 1 || modrm.mod == 2) {
    ...
    if (offset + 1 + disp_bytes > max_len) { // +1 wegen SIB
      snprintf(output, output_size, "%s[?]", size_prefix);
      return 0;
    }
    ...
    if (modrm.rm == 4) {
      ...
      int64_t disp = (disp_bytes == 1) ? *(int8_t *)(code + offset + 1)
      ...
      if (index == 4)
        snprintf(output, output_size, "%s[%s%s]", size_prefix,
                 get_reg_name(base, 8), disp_str);
      else
        snprintf(output, output_size, "%s[%s + %s*%d%s]", size_prefix,
                 get_reg_name(base, 8), get_reg_name(index, 8), scale,
                 disp_str);
    ...
    // simple reg + disp
    int64_t disp = (disp_bytes == 1) ? *(int8_t *)(code + offset)
                                     : *(int32_t *)(code + offset);
    char disp_str[32] = "";
    fmt_disp(disp_str, sizeof(disp_str), disp);

    snprintf(output, output_size, "%s[%s%s]", size_prefix, get_reg_name(rm, 8),
             disp_str);
    return disp_bytes;
  }
...
const char *get_group_mnemonic(uint8_t opcode, uint8_t reg_field) {
  ...
  if (opcode == 0xFF) {
    const char *grp5[] = {"inc", "dec",  "call", "callf",
                          "jmp", "jmpf", "push", "???"};
  ...
  if (opcode == 0x80 || opcode == 0x81 || opcode == 0x83) {
    const char *grp1[] = {"add", "or",  "adc", "sbb",
                          "and", "sub", "xor", "cmp"};
...
Instruction disasm_next(const uint8_t *code, size_t max_len,
                        uint64_t instr_addr) {
  ...
  if (is_two_byte) {
    ...
    if (opcode == 0xB6 || opcode == 0xB7 || opcode == 0xBE || opcode == 0xBF) {
      if (offset >= max_len) {
        snprintf(result.mnemonic, sizeof(result.mnemonic),
                 "movzx/movsx (no modrm, opcode=0x%02x)", opcode);
        result.size = offset;
        return result;
      }
      ...
      size_t extra = decode_rm_operand(
          code, offset, max_len, modrm, rex, src_size, instr_addr,
          0, // instr_len unknown yet → replaced later
          rm_str, sizeof(rm_str), true);
      offset += extra;

      // Now we know the full instr length → compute new rm_str including target
      // addr:
      decode_rm_operand(code, offset - extra, max_len, modrm, rex, src_size,
                        instr_addr,
      ...
      snprintf(result.mnemonic, sizeof(result.mnemonic), "%s %s, %s",
               (opcode == 0xB6 || opcode == 0xB7) ? "movzx" : "movsx",
               get_reg_name(reg, dst_size), rm_str);
    ...
    if (opcode >= 0x80 && opcode <= 0x8F) {
      ...
      offset += 4;
      const char *jcc[] = {"jo", "jno", "jb", "jae", "je", "jne", "jbe", "ja",
                           "js", "jns", "jp", "jnp", "jl", "jge", "jle", "jg"};
      snprintf(result.mnemonic, sizeof(result.mnemonic), "%s 0x%x",
               jcc[opcode & 0xF], rel);
      result.size = offset;
      return result;
    }
    ...
    if (opcode >= 0x90 && opcode <= 0x9F) {
      ...
      char rm_str[128];
      size_t extra_bytes =
          decode_rm_operand(code, offset, max_len, modrm, rex, 1, instr_addr, 0,
                            rm_str, sizeof(rm_str));
      offset += extra_bytes;

      snprintf(result.mnemonic, sizeof(result.mnemonic), "%s %s",
               setcc[opcode & 0xF], rm_str);
      result.size = offset;
      return result;
    }
    ...
    if (opcode == 0xAF) {
      ...
      char rm_str[128];
      size_t extra_bytes =
          decode_rm_operand(code, offset, max_len, modrm, rex, default_size,
                            instr_addr, 0, rm_str, sizeof(rm_str));
      offset += extra_bytes;

      snprintf(result.mnemonic, sizeof(result.mnemonic), "imul %s, %s",
               get_reg_name(reg, default_size), rm_str);
      result.size = offset;
      return result;
    }
    ...
    snprintf(result.mnemonic, sizeof(result.mnemonic), "unknown 0x0F 0x%02x",
             opcode);
    result.size = offset;
    return result;
  }
  ...
  if (opcode >= 0x50 && opcode <= 0x5F) {
    ...
    snprintf(result.mnemonic, sizeof(result.mnemonic), "%s %s", mnemonic,
             get_reg_name(reg, 8));
    result.size = offset;
    return result;
  }
  ...
  if (opcode == 0xE8 || opcode == 0xE9) {
    if (offset + 4 <= max_len) {
      int32_t rel = code[offset] | (code[offset + 1] << 8) |
      ...
      if (offset == 0) {
        snprintf(result.mnemonic, sizeof(result.mnemonic),
                 ...
                 target, (int)s.len, s.name);
      } else {
        snprintf(result.mnemonic, sizeof(result.mnemonic),
                 "%s 0x%llx -> <%.*s+0x%llx>",
                 (opcode == 0xE8) ? "call" : "jmp", target, (int)s.len, s.name,
                 offset);
      }
    } else {
  ...
  if (opcode == 0xEB || (opcode >= 0x70 && opcode <= 0x7F)) {
    if (offset < max_len) {
      ...
      const char *jcc[] = {"jo", "jno", "jb", "jae", "je", "jne", "jbe", "ja",
                           "js", "jns", "jp", "jnp", "jl", "jge", "jle", "jg"};
      const char *mnemonic = (opcode == 0xEB) ? "jmp" : jcc[opcode & 0xF];
  ...
  case 0xC1:
    ...
    is_group = true;
    operand_size =
        (opcode == 0xC0) ? 1 : default_size; // C0 = 8-bit, C1 = 16/32/64-bit
    break;
  case 0xFE:
  case 0xFF:
  case 0xF6:
  ...
  case 0xC7:
    ...
    is_group = true;
    operand_size =
        (opcode == 0xFE || opcode == 0x80 || opcode == 0xF6 || opcode == 0xC6)
            ? 1
            : default_size;
    break;
  default:
  ...
  if (needs_modrm) {
    ...
    if (is_group && (opcode == 0xC0 || opcode == 0xC1)) {
        ...
        char rm_str[128];
        size_t extra_bytes = decode_rm_operand(code, offset, max_len, modrm, rex,
                                               operand_size, instr_addr, 0,
                                               rm_str, sizeof(rm_str), true);
        offset += extra_bytes;
    ...
    if (opcode == 0xC6) {
      ...
      uint8_t imm = code[offset++];
      snprintf(result.mnemonic, sizeof(result.mnemonic), "movb %s, 0x%x",
               rm_str, imm);
      result.size = offset;
      return result;
    }
    ...
    if (is_group) {
      ...
      single_operand = (opcode == 0xFE || opcode == 0xFF || opcode == 0xF6 ||
                        opcode == 0xF7) &&
                       modrm.reg >= 2;
    }
    ...
    char rm_str[128];
    size_t extra_bytes =
        decode_rm_operand(code, offset, max_len, modrm, rex, operand_size,
                          instr_addr, 0, rm_str, sizeof(rm_str), true);
    offset += extra_bytes;

    if (single_operand) {
      snprintf(result.mnemonic, sizeof(result.mnemonic), "%s %s", base_mnemonic,
               rm_str);
    } else {
      snprintf(result.mnemonic, sizeof(result.mnemonic), "%s %s, %s",
               base_mnemonic, get_reg_name(reg, operand_size), rm_str);
    }
  ...
  default:
    snprintf(result.mnemonic, sizeof(result.mnemonic), "unknown 0x%02x",
             opcode);
    break;
  }
...
void disassemble_frame(uint64_t addr, size_t bytes) {
  ...
  while (offset < bytes) {
    ...
    Instruction ins = disasm_next(ptr + offset, bytes, instr_addr);
    Log::PrintLn("    %p: %s", (void *)instr_addr, ins.mnemonic);
    offset += ins.size;
  }
}
