...
void initialize_heap(void* heap_address, size_t page_count) {
    ...
    for (size_t i = 0; i < page_count; i++) {
        global_page_table_manager.map_memory(pos, global_allocator.request_page(), false);
...
void expand_heap(size_t length) {
    ...
    for (size_t i = 0; i < page_count; i++) {
        global_page_table_manager.map_memory(heap_end, global_allocator.request_page(), false);
