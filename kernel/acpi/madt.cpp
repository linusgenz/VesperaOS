        ...
        while (entries < end) {
            ...
            switch (static_cast<ACPI::MADTEntryType>(header->type)) {
                case ACPI::MADTEntryType::LOCAL_APIC: {
                    ...
                    if (const auto* entry = reinterpret_cast<ACPI::LocalAPICEntry*>(entries);
                ...
                case ACPI::MADTEntryType::X2APIC: {
                    ...
                    if (const auto* entry = reinterpret_cast<ACPI::X2APICEntry*>(entries);
                        (entry->flags & 0x1) || (entry->flags & 0x2)) {
                        if (cpu_count < MAX_CPU_CORES) {
                ...
                case ACPI::MADTEntryType::IO_APIC: {
                    const auto* entry = reinterpret_cast<ACPI::IOAPICEntry*>(entries);
