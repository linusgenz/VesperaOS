...
namespace cpu_manager {
    ...
    void initialize() {
        ...
        for (u32 i = 0; i < madt_cpu_count && i < kernel::acpi::madt::MAX_CPU_CORES; i++) {
            ...
            cpu_infos[i].state = CpuState::Offline;
            ...
            if (cpu_infos[i].is_bsp) {
                cpu_infos[i].state = CpuState::Online;
    ...
    void smp_init() {
        ...
        for (int i = 0; i < total_cpus; ++i) {
            ...
            int timeout = 0;
            while (!report->ready && timeout++ < 100) kernel::time::sleep_ms(10);
    ...
    void init_core(const CpuInfo* cpu) {
        arch::x86_64::interrupts::apic::send_init_ipi(cpu->apic_id);
        arch::x86_64::interrupts::apic::send_sipi(cpu->apic_id, SIPI_VECTOR);
    }

    CpuInfo* get_cpu_info_by_index(u32 index) {
        if (index >= total_cpus) return nullptr;
    ...
    u8 get_current_cpu_id() {
        ...
        const u32 current_apic_id = arch::x86_64::interrupts::apic::get_id();
