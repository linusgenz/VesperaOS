...
public:
    ...
    // bufferSize is not used to determine how much to read, but to assert, that the buffer is equal or greater than sectorCount * sector_size
    virtual ssize_t read(uint64_t lba, uint32_t sectorCount, void* buffer, size_t bufferSize) = 0;
    // bufferSize is not used to determine how much to write, but to assert, that the buffer is equal or greater than sectorCount * sector_size
