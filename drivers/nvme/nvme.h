...
namespace NVMe
{
    ...
    {
    public:
        NvmeNamespace(uint32_t nsid, NvmeQueue* ioQueue, const NVME_IDENTIFY_NAMESPACE_DATA* identify)
            : nsID(nsid), queue(ioQueue)
        {
            uint8_t lbaFormatIndex = identify->FLBAS.LbaFormatIndex;
            uint8_t lbads = identify->LBAF[lbaFormatIndex].LBADS;
            sectorSize = 1 << lbads;
