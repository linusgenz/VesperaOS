...
void usb_enable(void* arg) {
    ...
    if (const u8 vector = kernel::interrupts::get_free_vector();
        try_enable_msi_or_msix(reinterpret_cast<pci::PCI_HEADER0*>(pci_device_header), vector)) {
        char name[16];
        DeviceManager::alloc_unique_device_name("xhci", name, sizeof(name));

        const auto usb_driver = new usb::XhciDriver(vector, name, next_usb_bus_number.load());
...
namespace pci {
                    ...
                    case 0x03:
                        switch (pci_device_header->prog_if) {
                            ...
                            case 0x30: {
                                ...
                                char unit_name[32];
                                const u8 bus = next_usb_bus_number++;
                                snprintf(unit_name, sizeof(unit_name), "xhci%u", bus);
                                ...
                                const Unit* usb_unit =
                                    UnitManager::create(KERNEL_REALM_DRIVER, usb_enable, pci_device_header, &config);
                                if (!usb_unit) {
    ...
    void enumerate_device(const u64 bus_address, const u64 device) {
        ...
        for (u64 function = 0; function < 8; function++) enumerate_function(virt_raw(dev_virt), function);
    }
    ...
    void enumerate_bus(const u64 base_address, const u64 bus) {
        ...
        for (u64 device = 0; device < 32; device++) enumerate_device(virt_raw(bus_virt), device);
    }

    void enumerate_pci(kernel::acpi::MCFG_HEADER* mcfg) {
        next_usb_bus_number.init(0);
        const u32 entries = ((mcfg->header.length) - sizeof(kernel::acpi::MCFG_HEADER)) / sizeof(kernel::acpi::DEVICE_CONFIG);

        UsbManager::init();

        for (usize t = 0; t < entries; t++) {
            const auto* new_device_config = reinterpret_cast<kernel::acpi::DEVICE_CONFIG*>(
                reinterpret_cast<u64>(mcfg) + sizeof(kernel::acpi::MCFG_HEADER) + (sizeof(kernel::acpi::DEVICE_CONFIG) * t)
            );
