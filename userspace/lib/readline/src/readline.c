...
const char* rl_readline_name = "readline";

static void rl_write(const char* s, size_t n) {
    sys_write(HANDLE_STDOUT, (uint64_t)s, n, 0, 0, 0);
}
...
static int rl_readline_buf(const char* prompt, char* out, size_t out_size) {
    if (!out || out_size == 0) return -1;

    if (prompt) rl_write(prompt, strlen(prompt));

    // Switch HANDLE_STDIN to raw, no-echo mode.
    tty_mode_t saved, raw;
    tcgetattr(HANDLE_STDIN, &saved);
    raw.mode = TTY_MODE_RAW;
    raw.echo = 0;
    tcsetattr(HANDLE_STDIN, &raw);
    ...
    while (1) {
        char c = 0;
        int n = sys_read(HANDLE_STDIN, (uint64_t)&c, 1, 0, 0, 0);
        if (n  <= 0) {
        ...
        if (c == '\033') {
            char seq[4] = {0};
            sys_read(HANDLE_STDIN, (uint64_t)&seq[0], 1, 0, 0, 0);  // '['
            sys_read(HANDLE_STDIN, (uint64_t)&seq[1], 1, 0, 0, 0);  // letter or digit
            ...
            if (seq[0] == '[') {
                if (seq[1] == 'A') {
                    ...
                    // Delete key: ESC[3~
                    sys_read(HANDLE_STDIN, (uint64_t)&seq[2], 1, 0, 0, 0);  // '~'
                    if (cur < len) {
...
char* readline(const char* prompt) {
    char* buf = (char*)malloc(RL_MAX);
    if (!buf) return NULL;

    int n = rl_readline_buf(prompt, buf, RL_MAX);
    if (n < 0) {
        free(buf);
        return NULL;
    }
...
int readline_buf(const char* prompt, char* out, size_t out_size) {
    int n = rl_readline_buf(prompt, out, out_size);
    if (n > 0) add_history(out);
    return n;
}